...
class GithubRepositoryImp extends BaseRepositoryImp
    implements GithubUsersRepository {
  ...
  Future<Either<Failure, List<GithubUser>>> getGithubUsers({
    ...
    () => githubRemoteDataSource.getGithubUsers(query: query),
  );
  ...
  Future<Either<Failure, List<GithubUserDetails>>> getDetailedUsers(
    ...
    List<FutureEitherOr<GithubUserDetails>> requests = users
        .map(
          (user) =>
              () => performRemoteRequest(
                () => githubRemoteDataSource.getUserDetails(user.login),
              ),
        )
        .toList();
    return performMultipleRemoteRequests(requests);
  }
}
